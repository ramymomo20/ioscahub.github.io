    ...
    page.innerHTML = `
      <div class="grid cols-2">
        <div class="card" style="margin:0;">
          ...
          <div style="margin-top:10px;" class="footer-note">Registered: ${fmtDateTime(p.registered_at)} | Last active: ${fmtDateTime(p.last_active)}</div>
        </div>
        <div class="card" style="margin:0;">
      ...
      <div class="card" style="margin-top:10px;">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>Date</th><th>Match</th><th>Position</th><th>G</th><th>A</th><th>S</th><th>T</th><th>I</th></tr></thead>
            <tbody>
              ${recent.length ? recent.map((m) => `
                <tr>
                  ...
                  <td>${esc(m.goals || 0)}</td>
                  <td>${esc(m.assists || 0)}</td>
                  <td>${esc(m.keeper_saves || 0)}</td>
                  <td>${esc(m.tackles || 0)}</td>
                  <td>${esc(m.interceptions || 0)}</td>
                </tr>
